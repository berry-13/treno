...
export interface ConformalBucket {
  /** predictions with horizonSec <= maxHorizonSec use this band half-width */
...
export function applyResidual(model: ResidualModel, x: FeatureInput, p10: number, p50: number, p90: number): CorrectedPrediction {
  ...
  let off10: number;
  let off90: number;
  if (model.conformal && model.conformal.length > 0) {
    const horizonSec = x.schedArrEpoch != null ? Math.max(0, (x.schedArrEpoch - x.generatedAt) / 1000) : 1800;
    const bucket = model.conformal.find((b) => horizonSec <= b.maxHorizonSec) ?? model.conformal[model.conformal.length - 1]!;
    off10 = -bucket.offsetSec * 1000;
    off90 = bucket.offsetSec * 1000;
  } else {
    off10 = dot(model.pin10, row) * 1000;
    off90 = dot(model.pin90, row) * 1000;
  }
  return {
    p10: newP50 + Math.min(off10, -30_000),
