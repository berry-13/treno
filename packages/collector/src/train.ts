...
import { FEATURE_NAMES, featureRow, type FeatureInput, type ResidualModel } from './model.ts';
...
function main() {
  ...
  for (let i = 0; i < val.length; i++) {
    ...
    const mlo = Math.min(dot(pin10, Xva[i]!), -30);
    const mhi = Math.max(dot(pin90, Xva[i]!), 30);
    if (y - corr >= mlo && y - corr <= mhi) covModel++;
  }
  ...
  const covOk = covModel / val.length >= covHeur / val.length - 0.02;
  if (!improves || !covOk) {
    log.error('train: gate failed — model NOT deployed', { improves, covOk });
    process.exit(2);
  }
